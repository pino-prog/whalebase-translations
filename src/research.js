...
export async function buildGlossary(forceRebuild = false) {
  ...
  // 기존 용어집이 있으면 재사용 (--force 옵션 없을 시)
  if (!forceRebuild && existsSync(GLOSSARY_FILE)) {
    const existing = JSON.parse(await readFile(GLOSSARY_FILE, 'utf-8'));
    if (existing._researched_at) {
      const age = Date.now() - new Date(existing._researched_at).getTime();
      const days = Math.floor(age / (1000 * 60 * 60 * 24));
      if (days < 30) {
        console.log(`   기존 용어집 사용 (${days}일 전 조사, 30일마다 갱신)`);
        return existing;
      }
    }
  }
  ...
  const glossary = { _researched_at: new Date().toISOString() };

  for (let i = 0; i < LANGS.length; i++) {
    const lang = LANGS[i];
    console.log(`\n🔍 ${LANG_NAMES[lang]} 용어 조사 중...`);
    console.log(`   참고 서비스: ${REFERENCE_PLATFORMS[lang]}`);

    // 이전 언어의 기존 용어집이 있으면 유지
    const existing = (existsSync(GLOSSARY_FILE) && !forceRebuild)
      ? (JSON.parse(await readFile(GLOSSARY_FILE, 'utf-8').catch(() => '{}')))[lang]
      : null;
...
if (process.argv[1].endsWith('research.js')) {
  ...
  console.log('   웹 검색으로 Upbit, OKX, bitFlyer 등 실제 서비스 표현을 수집합니다.\n');
